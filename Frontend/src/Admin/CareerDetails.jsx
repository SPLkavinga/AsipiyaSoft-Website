...
function CareerDetails() {
  ...
  useEffect(() => {
    const fetchCareerDetails = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/career/${title}`);
        if (!response.ok) {
    ...
    <>
      ...
      <div
        ...
        <div className="z-10 w-full px-6 py-8 text-center md:w-2/3 md:px-20">
          <h2 className="text-4xl font-bold text-white md:text-5xl">{careerDetails.title}</h2>
          <p className="px-5 py-2 text-sm font-bold border-2 border-[#D9D9D9] text-gray-600 bg-white rounded-full">{careerDetails.job_type}</p>
        </div>
      </div>
...
<div className="mb-4 w-full">
  <span className="block mt-4 text-sm text-black md:text-lg break-words whitespace-pre-wrap">
    {careerDetails.description}
  </span>
</div>


          <h3 className="text-xl font-semibold mb-4 text-[#333333]">Responsibilities</h3>
